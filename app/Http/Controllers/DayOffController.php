...
class DayOffController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!empty($groupBy)) {
            ...
            $listAll = $query->select([
                ...
                ->map(function ($parent) use ($groupFormat, $request) {
                    ...
                    $parent->records = DayOffs::where('user_id', $parent->user_id)
        ...
        return view('pages.day_offs.index', compact(
            'listAll',
            'users',
            'typeDayOffEnum',
            'statusDayOffEnum',
    ...
    public function store(DayOffRequest $request)
    {
        ...
        try {
            ...
            return redirect()->route('day_offs.index')
                ->with(
                    ['message' => Lang::get('messages.day_off-create_s'), 'status' => 'success']
                );
        } catch (\Throwable $th) {
            DB::rollBack();
            throw $th;
    ...
    public function show(string $id)
    {
        ...
        return view('pages.day_offs.show', compact(
            'result',
            'users',
            'typeDayOffEnum',
            'typeSessionDayOffEnum',
    ...
    public function edit(string $id)
    {
        ...
        return view('pages.day_offs.edit', compact(
            'result',
            'users',
            'typeDayOffEnum',
            'typeSessionDayOffEnum',
    ...
    public function update(DayOffRequest $request, string $id)
    {
        ...
        try {
            ...
            return redirect()->route('day_offs.index')
                ->with(
                    ['message' => Lang::get('messages.day_off-update_s'), 'status' => 'success']
                );
        } catch (\Throwable $th) {
            DB::rollBack();
            throw $th;
    ...
    {
        //
        $record = DayOffs::find($id);
        $record->delete();
        return redirect()->route('day_offs.index')
            ->with(
                ['message' => Lang::get('messages.day_off-delete_s'), 'status' => 'success']
            );
    }
    ...
    /**
     * Send day_off request for approval and notify manager
     */
    public function send(string $id)
    {
        DB::beginTransaction();
        try {
            $day_off = DayOffs::findOrFail($id);
            ...
            // Get the manager of the user who created the day_off request
            $manager = User::find($day_off->user->manager_id);
            if ($manager) {
                // Create notification record for the manager
                Notifications::create([
                    'user_id' => $manager->id,
                    'title' => 'Yêu cầu ngày nghỉ',
                    'content' => 'Bạn có một yêu cầu ngày nghỉ cần duyệt từ ' . $day_off->user->name . ' (Mã: ' . $day_off->code . ')',
                    'link' => route('day_offs.show', $day_off->id),
            ...
            return redirect()->route('day_offs.index')
                ->with([
                    'message' => Lang::get('messages.day_off-send_s'),
                    'status' => 'success'
                ]);
        } catch (\Throwable $th) {
            DB::rollBack();
    ...
    public function approve(string $id)
    {
        ...
        try {
            ...
            $user = User::find($day_off->created_by);
            if ($user) {
                Notifications::create([
                    'user_id' => $user->id,
                    'title' => 'Yêu cầu ngày nghỉ đã được duyệt',
                    'content' => 'Yêu cầu ngày nghỉ của bạn đã được duyệt (Mã: ' . $day_off->code . ')',
                    'link' => route('day_offs.show', $day_off->id),
                    'is_read' => StatusNotifyReadEnum::UNREAD,
                    'type' => TypeNotifyReadEnum::DAY_OFF,
            ...
            return redirect()->route('day_offs.index')
                ->with([
                    'message' => Lang::get('messages.day_off-approve_s'),
                    'status' => 'success'
                ]);
        } catch (\Throwable $th) {
            DB::rollBack();
    ...
    /**
     * Reject day_off request and notify the requester
     */
        ...
        try {
            ...
            if ($user) {
                Notifications::create([
                    ...
                    'content' => 'Yêu cầu ngày nghỉ của bạn đã bị từ chối (Mã: ' . $day_off->code . ')',
                    'link' => route('day_offs.show', $day_off->id),
                    'is_read' => StatusNotifyReadEnum::UNREAD,
                    'type' => TypeNotifyReadEnum::DAY_OFF,
            ...
            return redirect()->route('day_offs.index')
                ->with([
                    'message' => Lang::get('messages.day_off-reject_s'),
                    'status' => 'success'
                ]);
        } catch (\Throwable $th) {
            DB::rollBack();
